...
//    data.append("region", payload.registerPayload.region);
    ...
    data.append("fcm_token", DEVICE_TYPE == 'Android' ? payload.registerPayload.deviceToken : null);
    data.append("apns_id", DEVICE_TYPE == 'IOS' ? payload.registerPayload.deviceToken : null);


    axios.post(BASE_URL + API.REGISTER_API, data, {
        headers: {
...
export const RegisterPatientDetailsApi = (payload, registerPatientDetailsResponse) => {
    let data = new FormData();
debugger
    let e1 = payload.emergency_contact1.slice(payload.emergency_contact1.lastIndexOf('+') + 1);
    let e2 = payload.emergency_contact2.slice(payload.emergency_contact2.lastIndexOf('+') + 1);
    ...
    axios.post(BASE_URL + API.REGISTER_PATIENT_DETAILS_API, data, {
        headers: {}
    }).then((response) => {
...
export const RegisterDoctorDetailsApi = (payload, registerDoctorDetailsResponse) => {
    let data = new FormData();
    debugger
    data.append("id", payload.id);
    ...
    data.append("certificate", {
        uri: Platform.OS === 'ios' ? payload.certificate.path.replace("file://", "/private") : payload.certificate.path,
        name: (payload.certificate.filename !== undefined && payload.certificate.filename !== null)
    ...
    data.append("image", {
        ...
        uri: Platform.OS === 'ios' ? payload.image.path.replace("file://", "/private") : payload.image.path,
        name: (payload.image.filename !== undefined && payload.image.filename !== null)
    ...
    if (payload.services_for.toLowerCase() === "internationals") {
        data.append("residential_permit", {
            uri: Platform.OS === 'ios' ? payload.residential_permit.path.replace("file://", "/private") : payload.residential_permit.path,
            name: (payload.residential_permit.filename !== undefined && payload.residential_permit.filename !== null)
                ? payload.timestamp + "residential_permit" + payload.residential_permit.filename
                : payload.timestamp + "residential_permit" + ".JPEG",
            type: payload.residential_permit.mime
        });
        data.append("work_permit", {
            uri: Platform.OS === 'ios' ? payload.work_permit.path.replace("file://", "/private") : payload.work_permit.path,
            name: (payload.work_permit.filename !== undefined && payload.work_permit.filename !== null)
    ...
    axios.post(BASE_URL + API.REGISTER_DOCTOR_DETAILS_API, data, {
        headers: {
