...
export default class VerifyOtpScreen extends BaseClass {
  ...
  handleVerifyOtpResponse = (response) => {
    ...
    if (response !== undefined && response !== null) {
      console.log("later", response);
      if (response.data.code === 200) {
        RegisterApi({ registerPayload }, (response) => {
          console.log("noweee", response, registerPayload);
    ...
    if (response !== undefined && response !== null) {
      ...
      if (response.code === 200) {
        const userProfile = {
          ...
          full_name:
            registerPayload.first_name + " " + registerPayload.last_name,
        };
        ConnectyCube.users
          .signup(userProfile)
          .then((user) => {
    ...
    return (
      <CardContainerStyle>
        ...
        <DescriptionText>
          ...
          shared on your mobile number below
        </DescriptionText>
        <Spacer space={5} />
        <OtpInputs
          clearTextOnFocus
            ...
            <ScrollContainer keyboardShouldPersistTaps={"handled"}>
              <MainContainer>
                ...
                <AbsoluteContainer>
                  ...
                  <Spacer space={8} />
                  <ResendText onPress={() => this.onResendOtp()}>
                    {STRINGS.RESEND_OTP_TEXT}
                  </ResendText>
                </AbsoluteContainer>
                <Spacer space={4} />
              </MainContainer>
