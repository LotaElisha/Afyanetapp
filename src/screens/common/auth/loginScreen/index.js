...
class LoginScreen extends BaseClass {
    ...
    if (loginResponse !== undefined && loginResponse !== null) {
      ...
      if (loginResponse !== simpleLoginData) {
        ...
        if (loginResponse.code === 200) {
          this.hideDialog();
          if (
            loginResponse.data.additional_details === undefined ||
            loginResponse.data.additional_details === null
          ) {
            if (
    ...
    if (socialResponse !== undefined) {
      ...
      if (socialResponse !== socialLoginData) {
        ...
        if (socialResponse.code === 200) {
          this.hideDialog();
          if (
            socialResponse.data.additional_details === undefined ||
            socialResponse.data.additional_details === null
          ) {
            if (
              socialResponse.data.user_data.user_type.toLowerCase() ===
                "patient" ||
              socialResponse.data.user_data.user_type.toLowerCase() ===
                "caretaker"
            ) {
              navigate("PatientDetailsScreen", {
                data: socialResponse.data.user_data,
              });
            } else {
              navigate("MedicalDetails", {
                data: socialResponse.data.user_data,
            ...
            socialResponse.data.payment_status === undefined ||
            (socialResponse.data.payment_status === null &&
              socialResponse.data.user_data.user_type.toLowerCase() ===
                "patient") ||
            socialResponse.data.user_data.user_type.toLowerCase() ===
              "caretaker"
            ...
            AsyncStorage.setItem(
              STRINGS.LOGIN_DATA,
              JSON.stringify(socialResponse.data)
            );
          ...
          if (this.twitterData.length !== 0) {
            ...
            socialData.push({
              name:
                this.twitterData.userName !== null
                  ? this.twitterData.userName
                  : "",
          ...
          if (this.googleData.length !== 0) {
            ...
            socialData.push({
              name:
                this.googleData.user.name !== null
                  ? this.googleData.user.name
                  : "",
  ...
  componentDidMount = () => {
    GoogleSignin.configure({
      iosClientId:
        "450742348040-grjdk0n12h5re2eo89ei045c4elpb4gj.apps.googleusercontent.com",
    });
    ...
    return (
      <ShadowViewContainer style={{ shadowRadius: 1 }}>
        <TouchableOpacity onPress={() => this.onAppleButtonPress()}>
          ...
          >
            <AntDesign name={"apple1"} color={COLORS.BLACK_COLOR} size={20} />
          </View>
        </TouchableOpacity>
      </ShadowViewContainer>
    );
  ...
  _renderSocialButtons = () => {
    return (
      <SocialButtonsContainer>
        ...
        {Platform.OS == "ios" && this._renderAppleButton()}
      </SocialButtonsContainer>
    );
  };
            ...
            >
              <MainContainer>
                ...
                <AbsoluteContainer>
                  ...
                  >
                    {STRINGS.DONT_HAVE_ACCOUNT_TEXT}{" "}
                    <RegisterText style={{ color: COLORS.APP_THEME_COLOR }}>
                      {STRINGS.REGISTER_NOW_TEXT}
                    </RegisterText>
                  </RegisterText>
                </AbsoluteContainer>
                <Spacer space={4} />
